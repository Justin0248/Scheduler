...
test("getInterviewersForDay returns an array wtih a length matching the number of interviewers for that day", ()=> {
...
test("getInterviewersForDay returns an empty array when the days data is empty", ()=> {
  const result = getInterviewersForDay(state, null);
  expect(result).toEqual([]);
});

test("getInterviewersForDay returns an empty array when the day is not found", ()=> {
  const result = getInterviewersForDay(state, undefined);
  expect(result).toEqual([]);
});